...
bool Props::img2wall(quint8 index, shared_ptr<GfxData> img)
{
    bool ret = true;
    ...
    Wall &wall = s->wall_vector[index].wall;

    /* Use another wall to track which sprites have been identified.
     * 0 = not identified (default value in constructor)
     * 1 = identified
     */
    Wall identified_sprites;

    /* Free sprites of wall being modified */
    for(int row=0; row<SCENERY_WALL_ROWS; row++)
        s->freeSprite(wall.sprite_idxs.at(row));
